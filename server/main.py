...
def load_df(json_path="name.json"):
    ...
    for column in ["Issues"]:
        ...
        column_as_df.columns = [
            str(column + "_" + subcolumn) for subcolumn in column_as_df.columns
        ]
    ...
    df = pd.DataFrame(
        [
            dict(y, index=i)
            for i, x in enumerate(df["Issues_Messages"].values.tolist())
            for y in x
        ]
    )
    ...
    def generate_completions(self, prefix_string, data, model_tf, tfidf_matrice):
        ...
        for i in range(len(similarity_scores)):
            similarity_scores[i][1][0] = (
                similarity_scores[i][1][0] * weights[similarity_indices][i]
            )
